...
\section{Assumptions and Problem Statement}
...
\par Say there are data streams $S_1,S_2,...,S_{n_1}$ and relations $R_1,R_2,...,R_{n_2}$. Let $S_i$ have $m_i$ attributes. Then total attributes are $a_{S_1,1}, a_{S_1,2},..., a_{S_1,m_1},..., a_{R_{n_2},m_{n_1+n_2}}$. 
\\ That is to say there are total of $\sum_{i=1}^{n_1+n_2}m_i=g$ attributes. We can use $1-0$ encoding so that any attribute or a combination in this schema can be represented by an array/vector $v$ of length $g$, 
