...
A database can be thought of as a list of tables, wherein each table itself can be considered as a list of data points ordered initially in the sequence they are entered.
\par Various tools can be used to connect to a database, here we focus on structured query languages(SQL). A simple SQL query looks like this
\begin{lstlisting}[language=SQL, caption=Example of SFW query for SQL]
    SELECT name, address
    FROM customer_details
    WHERE name="Tim"
\end{lstlisting}
This query is essentially asking to display the 2 columns from the table where the condition is given is satisfied. This particular query might be looking simple, but if the condition introduced is a complex one or if the table from which we need to return the output is complex, the question of how to execute the query optimally becomes difficult to answer.
...
\par The preprocessor also has to conduct semantic checking, that is, check if relations used exist, check for ambiguity, and type checking. If a parse tree passes the preprocessing then it is said to be \textbf{valid}.  In these parse trees, there are 2 types of nodes, one the atoms, which are essentially keywords in SQL, operators, constants, and attributes. The second is Syntactic categories, these are names for families of subqueries in triangular brackets. Each of the syntactic categories has a unique expansion into atoms and further syntactic categories.


\section{Relational algebra}
As we saw above, the order of operations matters, if the order of operations is not thought out and done blindly, a lot of redundant steps are executed and memory is moved around unnecessarily. There are few ways to at least look and analyze the operations and how they can be simplified.\cite{dbms}
...
\end{itemize}
When applying associative law on relations, one needs to be careful whether the conditions make sense after the order is changed.
\par While the above identities work on both sets and bags(bags allow for repetition). To show that laws for sets and bags do differ an easy way is to consider the distributive property.
\begin{center}
$A\cap _S(B\cup_S C) = (A\cap_S B)\cup_S(A\cap_S C)$\\
$A\cap _B(B\cup_B C) \neq (A\cap_B B)\cup_B(A\cap_B C)$
\end{center}
We can show it with an example. Let $A=\{t\},B=\{t\},C=\{t\}$. The LHS comes to be $\{t\}$, whereas RHS is $\{t,t\}$

\subsection{Select operator $\sigma$}
First, we start with the simple properties of the $\sigma$ operator. Need to be careful about the attributes used in the select operator condition when pushing it down.
\begin{itemize}
...
While for the Select operator($\sigma$) the identities were quite straightforward with not many things to consider, the identities for the Projection operator ($\pi$) are a bit more involved.
\begin{itemize}
...
Till now, the only SQL related information presented is how to convert a Query into the parse tree, which is grammar dependent. Given the parse tree, need to substitute nodes by operators seen above, this expression is optimized to be later converted to a physical query plan.
\par Now to convert the parse tree into the logical expression. First, look at the transformation of the select-from-where statement.
\begin{itemize}
...
\end{itemize}
While there is nothing inherently wrong with the last statement, need to consider the case where $<Condition>$ involves subqueries. A simple explanation about why it isn't allowed is a convention normally the subscript has to be a boolean condition, and if it was allowed otherwise, it would be a very expensive operation, as the subscript in the select operator has to evaluated at every element of the argument relation. This shows the redundancy of it. While if this is allowed, it can be simplified and made efficient but has to be done on a case by case basis with the use of $\bowtie, \times$ functions.
\par Overall it is a good idea to not use subquery and rather using joins.
\par At this point, by making the substitutions mentioned above and using the algebraic identities, we obtain a starting logical query plan. The query has to be transformed into a query that the compiler believes to be the cheapest or optimal. But, a thing that further complicates the process is the join order.
\par With the current knowledge, few optimizing rules are evident.
\begin{itemize}
    \item \textbf{Selection repositioning}, Selections should be pushed down as much as possible, but sometimes, might need to take the selection operator a level up first.
...
\end{itemize}
Finally after selecting all these methods need to check the time required to execute the plan, one obvious way to calculate the time is executing the plan to see the cost. But if we try to find the optimal plan by this method then we are essentially executing every plan. That is expensive and redundant. The next section presents few methods which can help in this task.
...
\par Compared to algorithms learned in an introductory algorithms class, here the memory usage is huge, and need to remember that intermediate/ temporary relations calculated will also incur a cost on the system. So need to take them into account as well because the physical plan is selected to minimize the estimated cost of evaluating the query. Overall the estimation method should pass the following sanity check:-
\begin{itemize}
...
$T(S)=\frac{T(R)}{V(R,A)}$, it is important to note that this is an estimate and not the actual value, this will be the actual value if all the attributes in $A$ have equal occurrence. An even better estimate can be obtained if the DBMS stores a statistic known as histogram. 
\par The above calculation was easy because of the equality. If $S=\sigma_{A < c}(R)$, we estimate $T(S)=\frac{T(R)}{3}$
...
$$\frac{T(R)*T(S)}{\text{max}(V(R,y_1), V(S,y_1))*\text{max}(V(R,y_2), V(S,y_2))}$$ 
...
\begin{itemize}
    \item Nested-loop join, where we assume the left argument is the relation of the outer loop.
...
\par Assuming there are no multidimensional indexes on several of the attributes, then each physical plan uses some number of attributes that each 
\begin{itemize}
    \item have an index
    \item are compared to a constant in one of the terms of the selection. 
\end{itemize}
...
User defined functions can be used to write SQL queries. Like any function in object oriented programming, UDF in SQL accept parameters of certain types as input and returns an output. These can help write queries quickly. UDFs can be modified independently of any other code.\\
SQL has the following types of UDFs:-
\begin{itemize}
    \item \textbf{Scalar functions}:- As the name suggests, the output is scalar, in the sense it is $1D$, only a single value is output. These functions always have a return clause in them. These functions traditionally do not provide high performance. 
    \item \textbf{Inline table-valued functions}:- These functions are optimized for performance.
    \item \textbf{Multi-statement table-valued functions}:- The syntax for these functions is similar to scalar fucntions, but these do allow for multiple outputs. The functions also do not provide high performance due to inner implementation.
   
\end{itemize}
In either of the above mentioned $3$ types, no changes to the data base can be made, e.g. no new tables can be created and only a certain amounts of manipulating syntax is allowed.

\subsection{Scalar}
Scalar functions are created using the syntax \textbf{CREATE FUNCTION}. They are deleted using the syntax \textbf{DELETE FUNCTION}. Functions can be modified using the syntax \textbf{ALTER FUNCTION}. Note, a restriction on Scalar functions is that they can not modify databases. An oversimplified way of looking at working of Scalar functions is:-
\begin{itemize}
    \item Take 0 or more arguments as input.
    \item Do computations based on the program written.
    \item Return as output a single scalar value.
\end{itemize}
The general syntax for a UDF is as follows:-
\begin{lstlisting}[language=SQL, caption= Scalar function syntax]
CREATE  FUNCTION [owner_name.]function_nam
...
END
\end{lstlisting} 
Function body normally contain control-of-flow statements and cursors, these are detrimental to the performance of the query and slow the execution down a lot.
\par When a Scalar UDF is used, the optimizer does not account for the cost of this function and treats it like a black box whereas it actually has some cost, there is a memory and time requirement. This not accounting for the costs of scalar functions lead to bad plans.
\par The Scalar UDF is treated as multiple different queries, so the queries written inside as executed one after another, so each statement has an independent execution plan which does not account for previous or next statement.
\par Scalar UDF also makes the system unable to run things in parallel.

\subsection{Inline}
\cite{tsql} Inline UDFs are a form of SQL view that accepts parameters. The param-eters are usually used in theWHEREclause to restrict the rows returned,but they can also be used in other parts of the UDF’sSELECTstatement.Inline UDFs can be convenient, but they don’t have all the features avail-able to views.
\par \cite{tsql} The essential fact of inline UDFs is that they are a type of view, one that allows for parameters that can be used in the single SELECT statement that makes up the body of the inline UDF. Note, a inline UDF is very similar to view.\\
The general syntax for a UDF is as given in \ref{lst:inlineFunctionSyntax}

\subsection{Multi Statement}
Multi statement UDFs are a useful hybrid of a scalar and inline UDF. They return a table that is constructed by the T-SQL script in the body of the function. Multistatement UDFs can’t execute stored procedures norcan they create or reference temporary tables or generate any messages. Their communication options are limited by design\cite{tsql}.\\
The lack of side effects is one of the features of multistatement UDFsthat distinguish them from stored procedures. In many ways, they are similar to a stored procedure that returns a single result set. If you can live with the restrictions, including the restriction on calling stored procedures, a multistatement UDF is a good substitute for a procedure.The lack of side effects improves the maintainability of the code. The primary reason for using a multistatement UDF instead of aninline UDF is the need for the program logic in the function body\cite{tsql}. The general syntax for Multistatement UDFs is given there\ref{lst:multistatementudf}

\subsection{Effects on Optimization}
Overall these functions can be treated as black box operations as they have various restrictions on them which normal SQL operators don't. There is no internal optimization for them, but rather consider them as a separate operator and decide when to execute them.
\par There is a lot of research currently being done on UDF \cite{froid}, \cite{ramachandra2019blackmagic}, \cite{gupta2020aggify}, these have improved the runtimes of Scalar UDF by huge amounts.
...
In the first section we said we proceed with the second strategy, i.e. monitor the metrics. Can extend that idea further to monitor the output of CQs to detect anomalies. Initially, Complex Event Processing was not a part of DSMS, but now many models have been developed to integrate the two. With the integration comes many additional functionalities and helps develop a theoretical base, but also puts additional burden on the system. This along with run time optimiations makes the implementation of DSMS quite complicated.
...
\par Reinforcement learning differs from supervised learning in not needing labelled input/output pairs be presented, and in not needing sub-optimal actions to be explicitly corrected. Instead the focus is on finding a balance between exploration (of uncharted territory) and exploitation (of current knowledge).
\par Reinforcement learning has been successful in applications as diverse as autonomous  helicopter  flight,  robot  legged  locomotion,  cell-phone  network routing, marketing strategy selection, factory control, and efficient web-pageindexing.  Our study of reinforcement learning will begin with a definition of the Markov decision processes (MDP), which provides the formalism in which RL problems are usually posed.\cite{solving_rl}
\subsection{Markov decision processes}
Markov decision process(MDP) is used to formalize various types of stochastic processes. In MDPs, the goal of the agent is to make a sequence of actions to optimize/ maximize an objective function \cite{Reinforcement_Learning}. \\
